  ...
  update(enemies: any, time: number, flowField?: any, hero?: any): void {
    ...
    if (nearestEnemy && distToNearest <= this.aggroRange && this.hasLineOfSight(nearestEnemy)) {
      ...
      if (distance <= this.attackRange) {
        ...
        // Use LOS shortcut: if we can see the enemy, charge directly
        this.moveToward(nearestEnemy);

        // Add Wall Avoidance and Separation
      ...
      // No enemies in aggro range? Follow the hero!
    ...
    // Animation state based on actual movement
    const actualSpeed = this.body ? Math.hypot(this.body.velocity.x, this.body.velocity.y) : 0;
    if (!this.isAttacking) {
      const anim = actualSpeed > 10 ? 'hero_run_anim' : 'hero_idle_anim';
      if (this.anims) {
        this.play(anim, true);
