...
const MapCanvas = ({ state, dispatch }) => {
  const areaRef = useRef(null);
  const mapCanvRef = useRef(null);
  const gridCanvRef = useRef(null);
  const curCanvRef = useRef(null);
  ...
  const drawMap = useCallback(() => {
    ...
    state.layers.forEach(layer => {
      ...
      Object.entries(layer.tiles).forEach(([key, tileRef]) => {
        if (!tileRef) return;
        const [tx, ty] = key.split(',').map(Number);
        const { x, y } = tw2s(tx, ty);
        const sheet = state.tilesets.find(s => s.id === tileRef.sheetId);
        if (!sheet?.img) return;
  ...
  // ── Draw cursor
  const drawCursor = useCallback((tx, ty) => {
  ...
  useEffect(() => {
    ...
    const panY = Math.round((h - state.mapH * state.tileSize * zoom) / 2);
    dispatch({ type: 'APPLY_FIT', zoom, panX, panY });
  }, [state._fitSig]);

  // ── Paint / Erase helpers
  const doPaint = useCallback((tx, ty) => {
    const sel = state.tsSel;
    const sheet = state.tilesets[state.activeTsIdx];
    if (!sel || !sheet) return;
    dispatch({
  ...
  const doErase = useCallback((tx, ty) => {
    dispatch({ type: 'ERASE_TILE', tx, ty });
  }, [dispatch]);

  const doFloodFill = useCallback((tx, ty) => {
    const sel = state.tsSel;
    const sheet = state.tilesets[state.activeTsIdx];
    if (!sel || !sheet) return;
  ...
  const doEyedrop = useCallback((tx, ty) => {
    if (!inMap(tx, ty)) return;
    const key = `${tx},${ty}`;
    for (let i = state.layers.length - 1; i >= 0; i--) {
      const tile = state.layers[i].tiles[key];
      if (tile && state.layers[i].visible) {
        const tsIdx = state.tilesets.findIndex(s => s.id === tile.sheetId);
        if (tsIdx >= 0) {
